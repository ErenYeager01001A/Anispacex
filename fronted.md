...
2. Copy provided component files into `src/components`.
3. Put all images you uploaded into `public/posters` and `public/player`.
4. Seed `public/seed/media/hls` with a short sample HLS (or fallback mp4).
5. Hook API calls to `src/lib/api.ts` (returning seed data until backend is ready).
6. Add Clerk provider and protect admin routes.
7. Run `pnpm dev` and test: open a Title detail → play player → click episodes.
